...
function selectSearchSearch() {
	if (!this.value) {
		this.parentNode.firstElementChild.selectedIndex = 0;
	}
}
...
function selectSearch(name) {
	...
	if (i == divs.length) {
		div.firstElementChild.value = name;
		div.firstElementChild.onchange();
	}
...
function skipOriginal(first) {
	var fnSelect = this.previousSibling.firstElementChild;
...
function selectClick(event, text, warning) {
	var td = this;
	var target = getTarget(event);
	if (!isCtrl(event) || isTag(td.firstElementChild, 'input|textarea') || isTag(target, 'a')) {
		return;
	}
	if (warning) {
		alert(warning);
	...
	var value = (td.firstElementChild && td.firstElementChild.alt) || td.textContent || td.innerText;
