...
class _Form extends \IPS\Helpers\Form
{
    ...
    public function setRandomOrder(): \IPS\Helpers\Form
    {
     ...
     * @return Element
     */
    public function addElement(string $name, string $type = 'text', string $custom = '', array $placement = []): Element
    {
    ...
    protected function compileTab(Element $element, &$lastTab)
    {
        ...
        if ($this->tabsToHeaders === false) {
            ...
            $this->addTabForm($tab, $options['icon'] ?? null, $options['blurblang'] ?? null, $options['css'] ?? null);
        } else {
    ...
    protected function compileHelper(Element $element)
    {
                        ...
                        case 'natogglesOff':
                            foreach ($toggle['elements'] as $v) {
                                if ($this->togglesAppending) {
                                    ...
                                    $options['togglesOff'][] = str_replace('.', '_', $v);
                                }
                            }
                            break;
                    }
        ...
        if (is_array(
                $options
            ) && isset($options['options']) && isset($options['parse']) && $options['parse'] === 'lang' && isset($options['prefixLang']) && $options['prefixLang']) {
            $langs = [];
...
//            $createdElement->description = $desc;
    ...
    protected function addHeaders($lang, $after = null, $tab = null, ?string $css = null, ?string $id = null){
        ...
        if ($after !== null) {
            $this->insertElement('after', $after, $lang, $element);
        } else {
            $this->elementStore[$lang] = $element;
        }
    ...
    public function addMessage(
        ...
        $element = (new Element($lang, 'message'))->extra(
            ['css' => $css, 'id' => $_id, 'parse' => $parse, 'sprintf' => $sprintf]
        );
    ...
    public function addSideBar($content, $prefix = true): self
    {
        $name = sha1($content);
        $element = new Element($content, 'sidebar');
        if ($prefix === false) {
            $element->skip();
        }
        $this->elementStore[$name] = $element;
