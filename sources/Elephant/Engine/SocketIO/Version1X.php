...
use IPS\toolbox\Elephant\Engine\AbstractSocketIO;
use IPS\toolbox\Elephant\Engine\Session;
use IPS\toolbox\Elephant\Exception\ServerConnectionFailureException;
use IPS\toolbox\Elephant\Exception\SocketException;
use IPS\toolbox\Elephant\Exception\UnsupportedTransportException;
use IPS\toolbox\Elephant\Payload\Encoder;
use IPS\toolbox\Elephant\Stream\AbstractStream;
use IPS\toolbox\Elephant\Stream\SequentialStream;
use IPS\toolbox\Elephant\Yeast;
 ...
 */
class _Version1X extends AbstractSocketIO
{
    const PROTO_OPEN = 0;
    const PROTO_CLOSE = 1;
    const PROTO_PING = 2;
    ...
    protected function createSocket()
    {
        ...
        $this->stream = AbstractStream::create(
            $this->url,
            $this->context,
            \array_merge($this->options, ['logger' => $this->logger])
        );
    ...
    protected function getUri($query)
    {
        ...
        return \sprintf('/%s/?%s', \trim($url['path'], '/'), \http_build_query($query));
    }
