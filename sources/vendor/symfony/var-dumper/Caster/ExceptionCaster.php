        ...
        for ($j += $trace->numberingOffset - $i++; isset($frames[$i]); ++$i, --$j) {
            ...
            if (isset($f[$prefix . 'src'])) {
                ...
                if ($trace->keepArgs && !empty($f['args']) && $frame instanceof EnumStub) {
                    $frame->value['arguments'] = new ArgsStub($f['args'], isset($f['function']) ? $f['function'] : null,
        ...
        if (isset($f['file'], $f['line'])) {
            ...
            if (isset(self::$framesCache[$cacheKey])) {
                ...
                $caller = isset($f['function']) ? sprintf('in %s() on line %d',
                    (isset($f['class']) ? $f['class'] . $f['type'] : '') . $f['function'], $f['line']) : null;
                $src = $f['line'];
                $srcKey = $f['file'];
                    ...
                    if (!empty($f['class']) && (is_subclass_of($f['class'],
                        ...
                        if (isset($templateInfo[$f['line']])) {
                            if (!method_exists($template,
                                    'getSourceContext') || !file_exists($templatePath = $template->getSourceContext()
                                                                                                 ->getPath())) {
                                $templatePath = null;
                            }
                            if ($templateSrc) {
                                $src = self::extractSource($templateSrc, $templateInfo[$f['line']], self::$srcContext,
                                    $caller, 'twig', $templatePath);
