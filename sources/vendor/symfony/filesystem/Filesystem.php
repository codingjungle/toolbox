...
class Filesystem
{
    ...
    public function mirror($originDir, $targetDir, Traversable $iterator = null, $options = [])
    {
        ...
        foreach ($iterator as $file) {
            if (false === strpos($file->getPath(), $originDir)) {
                throw new IOException(sprintf('Unable to mirror "%s" directory. If the origin directory is relative, try using "realpath" before calling the mirror method.',
                    $originDir), 0, null, $originDir);
            }
    ...
    public function exists($files)
    {
        ...
        foreach ($this->toIterable($files) as $file) {
            if (strlen($file) > $maxPathLength) {
                throw new IOException(sprintf('Could not check if file exist because path length exceeds %d characters.',
                    $maxPathLength), 0, null, $file);
            }
    ...
    public function remove($files)
    {
        ...
        foreach ($files as $file) {
            if (is_link($file)) {
                ...
                if (!(self::box('unlink', $file) || '\\' !== DIRECTORY_SEPARATOR || self::box('rmdir',
        ...
        foreach ($this->toIterable($dirs) as $dir) {
            ...
            if (!self::box('mkdir', $dir, $mode, true)) {
                if (!is_dir($dir)) {
                    ...
                    if (self::$lastError) {
                        throw new IOException(sprintf('Failed to create "%s": %s.', $dir, self::$lastError), 0, null,
                            $dir);
                    }
    ...
    private function linkException($origin, $target, $linkType)
    {
        if (self::$lastError) {
            if ('\\' === DIRECTORY_SEPARATOR && false !== strpos(self::$lastError, 'error code(1314)')) {
                throw new IOException(sprintf('Unable to create %s link due to error code 1314: \'A required privilege is not held by the client\'. Do you have the required Administrator-rights?',
                    $linkType), 0, null, $target);
            }
        }
        throw new IOException(sprintf('Failed to create %s link from "%s" to "%s".', $linkType, $origin, $target), 0,
            null, $target);
    }
    ...
    private function isReadable($filename)
    {
        ...
        if (strlen($filename) > $maxPathLength) {
            throw new IOException(sprintf('Could not check if file is readable because path length exceeds %d characters.',
                $maxPathLength), 0, null, $filename);
        }
