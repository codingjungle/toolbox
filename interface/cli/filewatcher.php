...
require_once str_replace('applications/dtproxy/interface/cli/filewatcher.php', '',
    ...
    public function check($args)
    {
        ...
        if ($hooks === 'hooks') {
            ...
            if (!file_exists($hookFile)) {
                ...
                if (!is_dir($fnpath) && mkdir($fnpath, IPS_FOLDER_PERMISSION, true) && is_dir($fnpath)) {
                    chmod($fnpath, IPS_FOLDER_PERMISSION);
                }
                if (is_dir($fnpath)) {
                    try {
                        $where = [
                        ...
                        $class = Db::i()->select('class', 'core_hooks', $where)->first();
                        $proxy = <<<EOF
<?php
